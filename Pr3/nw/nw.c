...
void runTest( int argc, char** argv) 
{
	...
	#pragma acc data copyin(input1[0:max_cols-1], input2[0:max_rows-1], blosum62[0:24][0:24]) copy(nw_matrix[0:max_rows*max_cols])

{

    int total_diagonals = (max_rows - 1) + (max_cols - 1) - 1;

    for (int k = 1; k <= total_diagonals; k++) {

        int start_i = MAXOF2(1, k - (max_cols - 2));

        int end_i = (k < max_rows - 1) ? k : (max_rows - 1);


        #pragma acc parallel loop gang vector private(S)
        ...
        for (int i = start_i; i <= end_i; i++) {
            ...
            if (j >= 1 && j < max_cols) {
                ...
                if(blosum) {
                    ...
                    S = blosum62[input1[j-1]][input2[i-1]];

                } else {

                    S = (input1[j-1] == input2[i-1]) ? 1 : -1;

                }

                int idx = i * max_cols + j;

                int match  = nw_matrix[(i-1) * max_cols + (j-1)] + S;

                int delet  = nw_matrix[i * max_cols + (j-1)] + penalty;

                int insert = nw_matrix[(i-1) * max_cols + j] + penalty;

                nw_matrix[idx] = MAXIMUM(match, delet, insert);
